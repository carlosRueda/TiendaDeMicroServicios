...
namespace TiendaDeMicroServicios.RabbitMQ.Bus.Implement
{
    public class RabbitEventBus : IRabbitEventBus
    {
            ...
            where TH : IEventoManejador<T>
        {
            ...
            if(_manejadores[eventoNombre].Any(x=>x.GetType() == manejadorEventoTipo))
                throw new ArgumentException($"El manejador {manejadorEventoTipo.Name} fue registrado por {eventoNombre}");
